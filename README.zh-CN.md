...
- CLI agent：`login`、`logout`、`status`、`sources list`、`sync --dry-run`、`sync`，支持 `tsk_` user API token/headless sync，并将本地 `deviceToken` 以 AES-GCM fallback 加密保存。
